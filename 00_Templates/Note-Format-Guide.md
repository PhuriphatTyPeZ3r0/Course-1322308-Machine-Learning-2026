...
# คู่มือ Format การสรุปโน้ตวิชา Machine Learning (1322308)
...
| --- | --- | --- | --- |
| `tags` | `[ml, weekN, <topic-tag>]` | `[ml, weekN, moc]` | topic-tag เลือก 1-2 คำที่สื่อเนื้อหา |
| `course` | `1322308` | `1322308` | ตายตัว ไม่ต้องเปลี่ยน |
...
| :--- | :--- | :--- | :--- |
| **Activity Diagram** | `flowchart TD` หรือ `LR` | ขั้นตอนอัลกอริทึม, ลูปการคำนวณ, Data Pipeline | เริ่มต้น `((●))`, กิจกรรม `([Action])`, เงื่อนไข `{Decision?}`, สิ้นสุด `(((●)))`, แยกระบบ `subgraph` |
| **Sequence Diagram** | `sequenceDiagram` | การส่งข้อมูลข้ามโมดูล, API Request/Response, Agent Interaction | `actor`, `participant`, `->>`, `-->>`, `alt/else`, `loop` |
| **State Machine Diagram** | `stateDiagram-v2` | วงจร Sequential, FSM, Lifecycle สถานะระบบ | `[*]`, สถานะ `State`, ทรานซิชัน `-->`, เงื่อนไข `[condition]` |
| **Class Diagram** | `classDiagram` | ความสัมพันธ์คลาส, สถาปัตยกรรมเชิงอ็อบเจกต์, Data Hierarchy | `class`, `+public`, `-private`, การสืบทอด `<\|--` |

> [!important] ข้อปฏิบัติในการเขียน Diagram
> - **Bilingual Text:** ข้อความอธิบายเป็นภาษาไทย พร้อมระบุศัพท์เทคนิคภาษาอังกฤษกำกับ (เช่น `([คำนวณเวกเตอร์ <br> (Compute Vector)])`)
